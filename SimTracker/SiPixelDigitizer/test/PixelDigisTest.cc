...
//     float pitchY = theGeomDet->specificTopology().pitch().second;
		...
		std::set<std::pair<int,int>> digiDcolLostFlagsTemp;
		int runOnFlags = src_.label() == "";
		if(runOnFlags)
		{
			const auto& digiDcolLostFlags = *DSViter;
			for(const auto& flags: digiDcolLostFlags)
			{
				if(flags.adc() == 0) continue;
				for(int d_col = -1; d_col <= 1; d_col++)
				{
					for(int d_row = -1; d_row <= 1; d_row++)
					{
						...
						if((flags.adc() & (1 << bit)) == 0) continue;
						digiDcolLostFlagsTemp.emplace(std::make_pair(flags.row() + d_row, flags.column() + d_col));
					}
				}
			}
		}
		...
		for(di = DSViter->data.begin(); di != DSViter->data.end(); di++)
		{
			...
			if(layer==1)
			{
				...
				if(!noise)
				{
					...
					if(runOnFlags)
					{
						for(const auto& neighbour: digiDcolLostFlagsTemp)
						{
							hpixMap1->Fill(neighbour.first, neighbour.second);
						}
					}
					else
					{
			...
			else if(layer==2)
			{
				...
				else
				{
					...
					if(runOnFlags)
					{
						for(const auto& neighbour: digiDcolLostFlagsTemp)
						{
							hpixMap2->Fill(neighbour.first, neighbour.second);
						}
					}
					else
					{
			...
			else if(layer==3)
			{
				...
				if(!noise)
				{
					...
					if(runOnFlags)
					{
						for(const auto& neighbour: digiDcolLostFlagsTemp)
						{
							hpixMap3->Fill(neighbour.first, neighbour.second);
						}
					}
					else
					{
